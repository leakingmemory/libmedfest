...
template <class T> static void PL(std::vector<T> &serializer, const T *deserializer, size_t num) {
...
template <class T, class S> static void PL(std::vector<T> &serializer, const S *deserializer, size_t num, const std::function<T (const S &)> &convert) {
    serializer.reserve(num);
    for (decltype(num) i = 0; i < num; i++) {
...
void FestDeserializer_0::Preload(FestSerializer &festSerializer) const {
    ...
    FestDeserializerPreloader_0 preloader{};
    if (versionMinor >= 2) {
...
void FestDeserializer_0::Preload(FestSerializer_0 &festSerializer) const {
    if (versionMajor > 0) {
