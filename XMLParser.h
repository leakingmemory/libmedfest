...
template <typename T> concept XMLObjectHandler = requires (T handler) {
    ...
    { handler.EndElement(std::declval<const std::shared_ptr<XMLObject>>()) };
};
...
public:
    ...
    virtual void EndElement(const std::shared_ptr<XMLObject> &obj) = 0;
};
...
public:
    ...
    void EndElement(const std::shared_ptr<XMLObject> &obj) override {
...
public:
    ...
    template <XMLObjectHandler Handler> void AddHandler(const std::string &name, const std::shared_ptr<Handler> &handler) {
        std::shared_ptr<XMLObjectHandlerInterface> interface = std::make_shared<XMLObjectHandlerGlue>(handler);
